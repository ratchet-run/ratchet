...
OWASP Dependency-Check fails its CI workflow at CVSS 7. Splitting its reports
does not waive findings or change dependency scopes in published POMs.
...
| `tests` | Nonpublished test/showcase/load applications, test-scope dependencies from other modules, and default dependency-plugin copied fixtures, including the mixed-version compatibility JARs |

All three reports enforce the same threshold. A library used in more than one
role appears in each applicable report. `provided` means that the host supplies
...
Reviewed on 2026-09-24. These dispositions describe the selected dependencies;
...
| CVE-2026-64607, CVE-2026-54399, CVE-2026-54428, CVE-2026-71290 | Apache HTTP code is embedded inside `docker-java-transport-zerodep`. Keep the findings in tooling/tests. Dependency-management overrides for ordinary Apache artifacts cannot replace relocated classes. A replacement transport release must contain the patched bytes and be qualified. |
...
No accepted-risk exceptions are introduced by these changes. Future suppressions
must identify the exact package/advisory mismatch and preserve an affected
