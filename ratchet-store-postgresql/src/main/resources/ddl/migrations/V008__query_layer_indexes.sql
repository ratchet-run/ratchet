...
--
-- Statements use plain CREATE INDEX (not CONCURRENTLY) because SchemaMigrator wraps each script
-- in a JDBC transaction and PostgreSQL rejects CREATE INDEX CONCURRENTLY inside a tx block.
-- Operators who need to avoid the brief table lock can pre-apply this migration manually with
-- CONCURRENTLY before enabling ratchet.schema.auto-migrate=true; the version table records V008
-- as applied either way.

CREATE INDEX IF NOT EXISTS idx_sj_traceparent
...
-- CREATE INDEX IF NOT EXISTS idx_archive_caller_principal
