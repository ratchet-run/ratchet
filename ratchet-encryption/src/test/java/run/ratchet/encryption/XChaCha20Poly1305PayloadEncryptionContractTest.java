...
/** Runs the engine-conformance contract against the XChaCha20-Poly1305 engine. */
class XChaCha20Poly1305PayloadEncryptionContractTest
    extends AbstractPayloadEncryptionEngineContract {
  ...
  private static EncryptionKey localKey(String id) {
    ...
    // The engine reads the encoded bytes, not the declared algorithm, so the tag is cosmetic.
