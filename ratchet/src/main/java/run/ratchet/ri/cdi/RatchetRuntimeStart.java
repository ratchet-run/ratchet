 ...
 *
 * <p>On a full Jakarta EE server the engine auto-starts on {@code @Initialized(ApplicationScoped.class)},
 * which the container fires only after the persistence unit and JTA are ready. Build-time-CDI
 * runtimes such as Quarkus/ArC fire {@code @Initialized(ApplicationScoped.class)} during STATIC_INIT
 * — before the {@code EntityManager} exists — so the engine's database-touching startup cannot run
 * there.
 *
 ...
 * fires this event from a later, post-persistence lifecycle event (for example Quarkus
 * {@code StartupEvent}). The engine's startup observers also observe this event, so firing it starts
 * the engine at the correct time. When the property is unset (the default, as on every Jakarta EE
