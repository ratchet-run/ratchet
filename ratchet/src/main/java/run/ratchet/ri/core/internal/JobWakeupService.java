...
public class JobWakeupService {
  ...
  public JobWakeupService(
    ...
    this.pollerSchedulerSupplier = pollerSchedulerSupplier;
  ...
  public JobWakeupService(
    ...
    this(
        clusterCoordinator,
        () -> pollerSchedulerInstance.isResolvable() ? pollerSchedulerInstance.get() : null,
        metricsCollector,
  ...
  private void publishNotification(JobPriority priority, String executionTarget) {
    AfterCommitRegistrar.Result result =
        afterCommitRegistrar.registerAfterCommit(
            () -> publishNotificationNow(priority, executionTarget));
    if (result == AfterCommitRegistrar.Result.NO_ACTIVE_TRANSACTION) {
      publishNotificationNow(priority, executionTarget);
    }
  }
  ...
  private void wakeupLocalPoller() {
    try {
      PollerScheduler scheduler = pollerSchedulerSupplier.get();
      if (scheduler == null) return;
      if (metricsCollector != null) metricsCollector.localWakeup("job_submit");
      scheduler.wakeup();
    } catch (Exception e) {
