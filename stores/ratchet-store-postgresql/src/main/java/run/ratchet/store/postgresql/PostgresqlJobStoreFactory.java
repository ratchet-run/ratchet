...
/** Creates ready-to-use PostgreSQL job stores without requiring CDI. */
public final class PostgresqlJobStoreFactory {

  private PostgresqlJobStoreFactory() {}

  /**
   * Creates and fully initializes a PostgreSQL job store.
   *
   ...
   * @return an initialized PostgreSQL job store
   */
  public static PostgresqlJobStore create(
    ...
    PostgresqlJobStoreImpl store =
        new PostgresqlJobStoreImpl(entityManagerProvider, metricsCollector, options);
    store.initialize();
    return store;
  }
}
