...
/** Creates ready-to-use SQL Server job stores without requiring CDI. */
public final class SqlserverJobStoreFactory {

  private SqlserverJobStoreFactory() {}

  /**
   * Creates and fully initializes a SQL Server job store.
   *
   ...
   * @return an initialized SQL Server job store
   */
  public static SqlserverJobStore create(
    ...
    SqlserverJobStoreImpl store =
        new SqlserverJobStoreImpl(entityManagerProvider, metricsCollector, options);
    store.initialize();
    return store;
  }
}
