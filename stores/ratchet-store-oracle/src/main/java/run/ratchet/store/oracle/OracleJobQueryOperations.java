...
/**
 * Dashboard-oriented search and count queries over the Oracle hot/cold store.
 *
 * <p>Uses the same LEFT JOIN projection as {@link OracleJobReadOperations} so that {@link
 * OracleJobRowMapper#hydrateJobEntity} can reconstruct both live and terminal jobs from a single
 ...
 */
final class OracleJobQueryOperations {

  private static final Logger log = Logger.getLogger(OracleJobQueryOperations.class);

  /*
   * Keep this builder dialect-local. It mirrors the PostgreSQL builder in shape, but the
   * common-looking clauses sit next to Oracle-specific hydration columns, trace extraction,
  ...
  /**
   * Archive rows projected to match {@link OracleJobRowMapper#HYDRATION_SELECT} column positions.
   ...
   * ({@link OracleJobRowMapper#HYDRATION_COL_COUNT}). The two SELECTs are combined with {@code
   * UNION ALL}; a column-count mismatch is rejected by Oracle before any row is returned, breaking
   * every archive-inclusive search. The leading two NULLs cover {@code payload}/{@code params},
   * which the archive does not retain.
   */
  // language=Oracle
  ...
  OracleJobQueryOperations(
      OracleStoreContext ctx, OracleJobRowMapper mapper, OracleTagOperations tags) {
    this.ctx = ctx;
    this.mapper = mapper;
    this.tags = tags;
  }
  ...
  List<JobEntity> searchJobs(JobFilter filter, int limit, int offset) {
    try {
      ...
      for (Object[] row : rows) {
        Object[] hydrationRow =
            archive ? Arrays.copyOf(row, OracleJobRowMapper.HYDRATION_COL_COUNT) : row;
        ...
        if (job != null) {
          ...
          // archive)
          if (!archive || row[OracleJobRowMapper.IDX_Q_STATUS] != null) {
  ...
  private void appendStatusCondition(JobFilter filter, StringBuilder where, List<Object> params) {
    ...
    for (JobStatus s : statuses) {
      if (OracleJobRowMapper.isLiveStatus(s)) {
        live.add(s);
      } else {
        terminal.add(s);
      }
  ...
  private static Set<JobStatus> terminalStatuses(Set<JobStatus> statuses) {
    return statuses.stream()
        .filter(s -> !OracleJobRowMapper.isLiveStatus(s))
  ...
  private void appendParentJobId(JobFilter filter, StringBuilder where, List<Object> params) {
    ...
    params.add(UuidRawConverter.toBytes(parentJobId));
  }
  ...
  private void appendCursorCondition(JobFilter filter, StringBuilder where, List<Object> params) {
    ...
    try {
      ...
      params.add(sortVal);
      params.add(sortVal);
      params.add(UuidRawConverter.toBytes(c.jobId()));
