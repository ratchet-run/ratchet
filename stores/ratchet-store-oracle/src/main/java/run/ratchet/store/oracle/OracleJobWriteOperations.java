...
final class OracleJobWriteOperations {
    ...
    try {
      ...
      if (bornTerminal) {
        ...
        if (hasBkey) {
          String ownerTable =
              recurring
                  ? OracleBusinessKeyReservations.OWNER_TABLE_RECURRING
  ...
  private void executeColdTerminalBackfill(JobEntity job, Timestamp nowTs) {
    ...
    ctx.em()
        ...
        .setParameter(5, UuidRawConverter.toBytes(job.getId()))
        .executeUpdate();
  }
  ...
  private void saveColdUpdate(JobEntity job) {
    ...
    ctx.em()
        .createNativeQuery(sql)
        .setParameter(1, OracleJobRowMapper.paramsToJson(job, active))
        .setParameter(
            2,
            OracleJobRowMapper.callbackPayloadToJson(
        ...
        .setParameter(4, UuidRawConverter.toBytes(job.getDependsOn()))
        .setParameter(5, UuidRawConverter.toBytes(job.getSupersededBy()))
        .setParameter(6, job.getResourceName())
        .setParameter(7, UuidRawConverter.toBytes(job.getId()))
        .executeUpdate();
  }
  ...
  private int bindColdInsert(Query q, JobEntity job, Timestamp nowTs, int i) {
    ...
    q.setParameter(i++, OracleJobRowMapper.payloadToJson(job, active));
    q.setParameter(i++, OracleJobRowMapper.paramsToJson(job, active));
    ...
    q.setParameter(
        ...
        OracleJobRowMapper.callbackPayloadToJson(
            job, job.getOnFailurePayload(), ProtectedSurface.ON_FAILURE_PAYLOAD, active));
    q.setParameter(i++, UuidRawConverter.toBytes(job.getDependsOn()));
    q.setParameter(i++, UuidRawConverter.toBytes(job.getSupersededBy()));
    q.setParameter(i++, nowTs);
    q.setParameter(i++, job.getCallerPrincipal());
    q.setParameter(i++, OracleJobRowMapper.traceContextToJson(job));
    q.setParameter(i++, UuidRawConverter.toBytes(job.getRecurringMasterId()));
  ...
  private boolean tryScheduledTimeOnlyHotUpdate(JobEntity job, Object[] row) {
    ...
    String updateSql =
        ...
        SET scheduled_time = ?, updated_at = CAST(SYS_EXTRACT_UTC(SYSTIMESTAMP) AS TIMESTAMP)
  ...
  private void updateHotLiveViaVersion(JobEntity incoming, int expectedVersion) {
    ...
    int updated =
        ctx.em()
            ...
            .setParameter(8, UuidRawConverter.toBytes(id))
