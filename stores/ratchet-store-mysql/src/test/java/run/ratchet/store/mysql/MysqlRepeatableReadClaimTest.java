...
class MysqlRepeatableReadClaimTest {
  ...
  void emptyRecurringClaimDoesNotBlockRegistration() throws Exception {
    ...
    try {
      ...
      assertTrue(store.claimDueRecurring(10, "reader", NodeTagFilter.NONE).isEmpty());
      var insert =
          workers.submit(
              () -> {
  ...
  void oldSnapshotCannotExceedResourceCapacity() {
    var permits = fixture.store().capability(ResourcePermitStore.class).orElseThrow();
  ...
  void skipsLockedHeadAndFindsLaterCandidates() {
    ...
    firstJob.setScheduledTime(Instant.now().minusSeconds(10));
    ...
    try {
      ...
      locked
          ...
          .setParameter(1, UuidByteArrayConverter.toBytes(first.getId()))
          .getResultList();
  ...
  void concurrentInsertCannotDuplicateClaimAcrossReadCommittedPages() {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      job.setScheduledTime(Instant.now().minusSeconds(60 - i * 20));
    ...
    try {
      ...
      blocker
          .createNativeQuery("SELECT job_id FROM scheduler_job_queue WHERE job_id=? FOR UPDATE")
          .setParameter(1, UuidByteArrayConverter.toBytes(jobs.get(0).getId()))
          .getResultList();
      ...
      EntityManager observed =
          afterFirstPointLookup(
              ...
              () -> {
                ...
                inserted.setPriority(JobPriority.CRITICAL);
      ...
      assertEquals(
          List.of(jobs.get(1).getId(), jobs.get(2).getId()),
  ...
  void concurrentSmallQueueClaimsDoNotScanOtherOwnersRows() throws Exception {
    ...
    var bothLocked = new CyclicBarrier(2);
            ...
            workers.submit(
                () -> {
                  ...
                  try {
                    ...
                    return claims.stream().map(JobClaimDto::id).toList();
                  } finally {
                    if (em.getTransaction().isActive()) em.getTransaction().rollback();
                    em.close();
                  }
