...
/**
 * JSON-P encoder/decoder for the JMS coordinator's wire envelope.
 *
 * <p>Envelope shape mirrors the PostgreSQL coordinator so operators tcpdumping the network see the
 * same JSON whether they're inspecting a PG NOTIFY or a JMS TextMessage body:
 *
 * <pre>{@code
 * {"v":1,"node":"<NodeIdentity.value>","prio":"HIGH|NORMAL|LOW|LOWEST|CRITICAL"}
 * }</pre>
 *
 * <p>Forward-compat rules are identical to the PG codec — unknown fields are ignored, future
 * versions raise {@link DecodeException}, and unknown priority strings are rejected so a sender
 * typo doesn't silently fall back to {@code NORMAL} on the receiver.
 *
 * <p>JSON-P only — no Jackson, no JSON-B. Instances are thread-safe; a fresh reader/writer is
 * allocated per call.
 */
final class JmsNotifyPayloadCodec {
  ...
  /**
   * Decoded envelope passed between the message listener and dispatch path. {@code cid} is an
