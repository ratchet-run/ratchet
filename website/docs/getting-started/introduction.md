...
| **Pluggable store SPI** | Use MySQL, PostgreSQL, Oracle, SQL Server, or MongoDB. Or implement the TCK and bring your own. |
...
Ratchet is currently at version **0.3.2-SNAPSHOT**. The core API is stabilizing, but interfaces marked with `@Incubating` (such as `CircuitBreakerProtected` and `CircuitBreakerProfile`) may change in future releases. Feedback and contributions are welcome.
