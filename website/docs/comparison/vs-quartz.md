...
Ratchet ships a `JobStore` SPI with three reference implementations (MySQL, PostgreSQL, MongoDB) that all pass the same TCK. The DDL is plain SQL files you can apply with whatever migration tool you already use (Flyway, Liquibase, plain `psql`). You can also opt in to a `SchemaMigrationLifecycleHook` that runs at startup if you want zero-config.
...
1. Job execution can authenticate as the original caller (useful for tenant-scoped data access).
