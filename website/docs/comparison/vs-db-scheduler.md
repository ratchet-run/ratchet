...
Ratchet uses a multi-table schema (jobs, batches, recurring jobs, archived jobs, locks, signals, etc.) and supports MySQL, PostgreSQL, and MongoDB via a pluggable `JobStore` SPI with a TCK. The schema is bigger, the model is richer, and the trade-off is real: more code, more tables, more to understand.
