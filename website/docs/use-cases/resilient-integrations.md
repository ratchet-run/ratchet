...
::: tip Verified
The Java on this page compiles against `ratchet-api` `0.4.1-SNAPSHOT`. It shows real API usage, not pseudocode. The running app needs a Jakarta EE server and a configured store. The `@CircuitBreakerProtected` annotation is marked `@Incubating` and may change.
:::
