...
1. **Job claiming is safe by default.** The database handles concurrency via row-level locking (`SELECT ... FOR UPDATE SKIP LOCKED` on PostgreSQL and Oracle, InnoDB row locks on MySQL, `UPDLOCK, READPAST` row locks on SQL Server, atomic updates on MongoDB). No additional configuration is needed for one-shot jobs.
