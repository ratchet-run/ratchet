...
class SqlserverDependencyVersionTest {

  @Test
  void resolvesPinnedTestcontainersMssqlServer205() throws Exception {
    Path artifact =
        Path.of(
            MSSQLServerContainer.class.getProtectionDomain().getCodeSource().getLocation().toURI());

    assertThat(artifact.getFileName().toString()).isEqualTo("testcontainers-mssqlserver-2.0.5.jar");
