...
import run.ratchet.spring.boot.it.sqlserver.fixture.tck.TckApplication;
import run.ratchet.spring.boot.it.sqlserver.fixture.tck.TckApplicationContextInitializer;
import run.ratchet.store.converter.PayloadSerializerHolder;
import run.ratchet.tck.api.ListenerProbe;

/** End-to-end proof that the starter owns a complete SQL Server scheduler runtime. */
@ExtendWith(OutputCaptureExtension.class)
public class SqlserverSchedulerRuntimeTest extends SqlserverIntegrationTestSupport {
  ...
  void runnerExecutesAfterMigratedRuntimeStartAndContextCloseDrains(CapturedOutput output) {
    ...
    try (ConfigurableApplicationContext context =
      ...
      int migrationIndex = sqlProbe.lastIndexContaining("merge ratchet_schema_version");
      int nodeStartIndex = sqlProbe.firstIndexContaining("merge scheduler_node");
  ...
  static class RuntimeTestConfiguration {
    ...
    ApplicationRunner qualifyingRunner(
      ...
      return arguments -> {
        ...
        RuntimePayload payload = new RuntimePayload("spring-sqlserver", 8);
