...
/** Beans replacing store time and storage for the clock-dependent contract. */
...
public class ClockedTckConfiguration {
        ...
        .security(
            security ->
                security.classPolicyAllowedPackages(
                    StoreTckBindings.binding().allowedPackages()))
        .build();
  }
  ...
  SpringClockedTckRuntime springClockedTckRuntime(
    ...
    return new SpringClockedTckRuntime(
        ...
        clock,
        jobStore,
        StoreTckBindings.binding().clockedRuntimeName());
  }
}
