...
public final class SpringRatchetTckRuntime implements RatchetTckRuntime {
  ...
  private final StoreTckBinding binding;
  ...
  public boolean supportsCallerTransactionRollback() {
    return binding.supportsCallerTransactionRollback();
  }
  ...
  private void clearStore() {
    for (int attempt = 1; ; attempt++) {
      try {
        ...
        if (attempt >= binding.storeClearAttempts()
            || !binding.isRetryableStoreClearFailure(failure)) {
          throw failure;
        }
        try {
          Thread.sleep(binding.storeClearRetryBackoffMillis());
