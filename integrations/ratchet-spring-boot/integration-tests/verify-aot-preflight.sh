#!/usr/bin/env bash
#
# verify-aot-preflight.sh — run the Boot 4.1 JVM control and native AOT
# payload preflight against artifacts installed by the same verify.sh run.
...
POM="$ROOT/integrations/ratchet-spring-boot/integration-tests/aot-preflight/pom.xml"
TOOLCHAIN="$ROOT/integrations/ratchet-spring-boot/integration-tests/native-postgresql/native-toolchain.properties"
EVIDENCE_DIR="$ROOT/integrations/ratchet-spring-boot/integration-tests/aot-preflight/target/spring-boot-aot-evidence"
...
IMAGE_NAME="ratchet-aot-preflight:verify-$$"

for command in docker java mvn python3 tee; do
...
PY
  ...
  echo "verify-aot-preflight.sh: $MAVEN_REPO" >&2
  exit 1
}
...
for required_file in "$MATRIX" "$POM" "$TOOLCHAIN"; do
  if [[ ! -f "$required_file" ]]; then
    echo "required preflight input is missing: $required_file" >&2
    exit 1
  fi
done
...
PY
  ...
  echo "verify-aot-preflight.sh: $properties" >&2
  exit 1
}
...
PY
    ...
    echo "verify-aot-preflight.sh: $CONSUMER_JAVA_HOME" >&2
    exit 1
  }
...
echo "Running Boot 4.1 JVM-control preflight on Java from ${CONSUMER_JAVA_HOME:-PATH}..."
if [[ -n "$CONSUMER_JAVA_HOME" ]]; then
...
echo "Building Boot 4.1 native preflight image with pinned Paketo references..."
set +e
...
if [[ $build_status -ne 0 ]]; then
  echo "native preflight image build failed with exit code $build_status" >&2
  exit "$build_status"
fi
...
echo "Running native preflight image..."
set +e
docker run --rm "$IMAGE_NAME" 2> >(tee "$RUN_STDERR_LOG" >&2) | tee "$RUN_LOG"
...
prefix = "SPRING_BOOT_AOT_EVIDENCE="
required = [
    "method-references",
    "wrapper-lambda",
    "jsonb-payload-round-trip",
    "job-payload-invocation",
    "manifest-rejection",
]
evidence = {}
errors = []
...
if errors:
    raise SystemExit("native AOT evidence failed validation: " + "; ".join(errors))
print(f"Validated five native AOT scenarios: {evidence_path}")
PY

echo "Spring Boot AOT preflight passed."
