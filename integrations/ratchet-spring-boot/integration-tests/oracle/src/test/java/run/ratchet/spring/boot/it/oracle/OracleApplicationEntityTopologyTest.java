...
import run.ratchet.spring.boot.it.oracle.fixture.application.ApplicationEntityApplication;
import run.ratchet.spring.boot.it.oracle.fixture.application.ConsumerNote;
import run.ratchet.store.entity.JobEntity;

class OracleApplicationEntityTopologyTest extends OracleIntegrationTestSupport {

  @Test
  void applicationAndRatchetEntitiesShareOnePersistenceUnitAndTransactionManager()
      throws Exception {
    executeSql(
        "CREATE TABLE consumer_note (id RAW(16) PRIMARY KEY, message VARCHAR2(255) NOT NULL)");
    ...
    contextRunner(ApplicationEntityApplication.class, migrationOptions(""))
        .run(
            context -> {
              ...
              assertThat(
                      queryForLong(
                          "SELECT COUNT(*) FROM consumer_note WHERE id = HEXTORAW('"
                              + id.toString().replace("-", "")
                              + "')"))
                  .isEqualTo(1L);
              assertThat(
