 ...
 * Oracle {@code TIMESTAMP(6)} column back through Hibernate 7's native-query path must recover the
 * exact instant that was written, not a value shifted by the JVM default zone offset.
 */
class OracleInstantRoundTripTest extends OracleIntegrationTestSupport {
  ...
  @Test
  void instantRoundTripsExactlyThroughOracleTimestampColumn() {
    contextRunner(RatchetOnlyApplication.class, migrationOptions("oracle"))
        .run(
            context -> {
              OracleJobStore store = store(context);
