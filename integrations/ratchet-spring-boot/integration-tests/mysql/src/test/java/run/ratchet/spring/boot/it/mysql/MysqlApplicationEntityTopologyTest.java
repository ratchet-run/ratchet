...
import run.ratchet.spring.boot.it.mysql.fixture.application.ApplicationEntityApplication;
import run.ratchet.spring.boot.it.mysql.fixture.application.ConsumerNote;
import run.ratchet.store.entity.JobEntity;

class MysqlApplicationEntityTopologyTest extends MysqlIntegrationTestSupport {

  @Test
  void applicationAndRatchetEntitiesShareOnePersistenceUnitAndTransactionManager()
      throws Exception {
    executeSql(
        "CREATE TABLE consumer_note (id BINARY(16) PRIMARY KEY, message VARCHAR(255) NOT NULL)");
    ...
    contextRunner(ApplicationEntityApplication.class, migrationOptions(""))
        .run(
            context -> {
              ...
              assertThat(
                      queryForLong(
                          "SELECT COUNT(*) FROM consumer_note WHERE id = UUID_TO_BIN('"
                              + id
                              + "')"))
                  .isEqualTo(1L);
              assertThat(
              ...
              assertThat(
                      context
                          ...
                          .countContaining("SELECT @@SESSION.transaction_isolation"))
                  .isEqualTo(1L);
            });
  }
}
