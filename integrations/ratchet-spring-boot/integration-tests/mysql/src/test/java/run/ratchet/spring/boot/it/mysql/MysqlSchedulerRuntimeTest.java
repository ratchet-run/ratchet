...
import run.ratchet.spring.boot.it.mysql.fixture.tck.TckApplication;
...
/** End-to-end proof that the starter owns a complete MySQL scheduler runtime. */
@ExtendWith(OutputCaptureExtension.class)
public class MysqlSchedulerRuntimeTest extends MysqlIntegrationTestSupport {
  ...
  void runnerExecutesAfterMigratedRuntimeStartAndContextCloseDrains(CapturedOutput output) {
    ...
    try (ConfigurableApplicationContext context =
      ...
      int migrationIndex = sqlProbe.lastIndexContaining("insert into ratchet_schema_version");
      int nodeStartIndex = sqlProbe.firstIndexContaining("insert into scheduler_node");
  ...
  static class RuntimeTestConfiguration {
          ...
          Long schedulerTableCount =
              jdbc.queryForObject(
                  "SELECT COUNT(*) FROM information_schema.tables"
                      + " WHERE table_schema = 'ratchet_spring_boot'"
    ...
    ApplicationRunner qualifyingRunner(
      ...
      return arguments -> {
        ...
        RuntimePayload payload = new RuntimePayload("spring-mysql", 8);
