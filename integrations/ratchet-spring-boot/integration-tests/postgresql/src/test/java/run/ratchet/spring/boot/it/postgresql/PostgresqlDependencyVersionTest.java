...
class PostgresqlDependencyVersionTest {

  @Test
  void resolvesPinnedTestcontainersPostgresql205() throws Exception {
    Path artifact =
        Path.of(
            PostgreSQLContainer.class.getProtectionDomain().getCodeSource().getLocation().toURI());

    assertThat(artifact.getFileName().toString()).isEqualTo("testcontainers-postgresql-2.0.5.jar");
