...
class PostgresqlMigrationTruthTest extends PostgresqlIntegrationTestSupport {
            ...
            context -> {
              ...
              SchemaMigrator migrator =
                  new SchemaMigrator(
                      ...
                      context.getBean(PostgresqlSchemaMigrationDialect.class));
  ...
  void autoMigrateDisabledDoesNotCreateAnySchemaObjects() {
    contextRunner(RatchetOnlyApplication.class, noMigrationOptions())
        ...
        .run(
            context -> {
              assertThat(context).hasNotFailed();
              assertThat(context).hasSingleBean(PostgresqlJobStore.class);
  ...
  @Test
  void blankDialectInfersPostgresql() {
    contextRunner(RatchetOnlyApplication.class, migrationOptions("   "))
        .run(
            context -> {
              assertThat(context).hasNotFailed();
              assertThat(context).hasSingleBean(PostgresqlSchemaMigrationDialect.class);
        ...
        .run(
            context -> {
              ...
              assertThat(failureMessages(context.getStartupFailure()))
                  .contains(
                      "requires ratchet.schema.migration-dialect to be blank or 'postgresql',"
                          + " but was");
            });
  }
  ...
  void migrationFailureFailsStartup() {
    contextRunner(MigrationFailureApplication.class, migrationOptions("postgresql"))
        .run(
            context -> {
              assertThat(context).hasFailed();
              assertThat(failureMessages(context.getStartupFailure()))
                  .contains("Ratchet PostgreSQL schema auto-migration failed")
                  .contains("Intentional PostgreSQL migration failure");
            });
  }
    ...
    contextRunner(RatchetOnlyApplication.class, migrationOptions(""))
        .run(
            context -> {
              assertThat(context).hasNotFailed();
              assertThat(context).hasSingleBean(PostgresqlJobStore.class);
              assertThat(context).hasBean("ratchetJpaSchemaMigrationInitializer");
              ...
              int storeInitialization = probe.firstIndexContaining("SHOW transaction_isolation");
              ...
              assertThat(probe.countContaining("SHOW transaction_isolation")).isEqualTo(1L);
            });
  }
