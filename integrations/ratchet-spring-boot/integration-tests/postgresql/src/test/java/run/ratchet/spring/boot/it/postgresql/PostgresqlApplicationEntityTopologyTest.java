...
import run.ratchet.spring.boot.it.postgresql.fixture.application.ApplicationEntityApplication;
import run.ratchet.spring.boot.it.postgresql.fixture.application.ConsumerNote;
import run.ratchet.store.entity.JobEntity;

class PostgresqlApplicationEntityTopologyTest extends PostgresqlIntegrationTestSupport {

  @Test
  void applicationAndRatchetEntitiesShareOnePersistenceUnitAndTransactionManager()
      throws Exception {
    executeSql("CREATE TABLE consumer_note (id UUID PRIMARY KEY, message VARCHAR(255) NOT NULL)");
    ...
    contextRunner(ApplicationEntityApplication.class, migrationOptions(""))
        .run(
            context -> {
              ...
              assertThat(
                      queryForLong(
                          "SELECT COUNT(*) FROM consumer_note WHERE id = '" + id + "'::uuid"))
                  .isEqualTo(1L);
              assertThat(
                      queryForLong(
              ...
              assertThat(
                      context
                          .getBean(SqlStatementProbe.class)
                          .countContaining("SHOW transaction_isolation"))
                  .isEqualTo(1L);
            });
  }
}
