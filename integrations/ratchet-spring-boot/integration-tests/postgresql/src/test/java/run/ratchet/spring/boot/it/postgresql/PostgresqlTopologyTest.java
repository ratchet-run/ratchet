...
class PostgresqlTopologyTest extends PostgresqlIntegrationTestSupport {
    ...
    contextRunner(RatchetOnlyApplication.class, migrationOptions(""))
        .run(
            context -> {
              ...
              PostgresqlJobStore store = store(context);
              assertThat(AopUtils.isAopProxy(store)).isTrue();
              Class<?> targetClass = AopUtils.getTargetClass(store);
              Method create = PostgresqlJobStore.class.getMethod("create", JobEntity.class);
