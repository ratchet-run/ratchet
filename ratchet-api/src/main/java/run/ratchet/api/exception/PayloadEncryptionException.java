 ...
 * and {@link KeyNotFoundException} are poison data that no retry can fix and route to the
 * controlled failure path (DLQ); {@link KeyProviderUnavailableException} is a transient
 * infrastructure fault that is retried with backoff; {@link EncryptionConfigurationException} is an
 * operator mistake that fails the node at startup. Code that only needs to know "an encryption
 * operation failed," without caring which kind, can catch this base type.
 *
