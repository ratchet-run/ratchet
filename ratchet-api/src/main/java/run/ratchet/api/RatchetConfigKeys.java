...
final class RatchetConfigKeys {
  ...
  static final RatchetConfigKey<Long> NODE_HEARTBEAT_INTERVAL_SECONDS =
      longKey(
          ...
          10L,
          1L);
  static final RatchetConfigKey<Long> NODE_ORPHAN_GRACE_SECONDS =
  ...
  static final RatchetConfigKey<Integer> RECURRING_BATCH_LIMIT =
      intKey("ratchet.recurring.batch-limit", "RATCHET_RECURRING_BATCH_LIMIT", 20, 1);
  static final RatchetConfigKey<Long> RECURRING_POLL_MS =
      longKey("ratchet.recurring.poll-ms", "RATCHET_RECURRING_POLL_MS", 1000L, 1L);
  static final RatchetConfigKey<Long> RECURRING_MAX_POLL_MS =
      longKey("ratchet.recurring.max-poll-ms", "RATCHET_RECURRING_MAX_POLL_MS", 60000L, 1L);
  static final RatchetConfigKey<Long> RECURRING_STARTUP_GRACE_SECONDS =
      longKey(
          "ratchet.recurring.startup-grace-seconds",
          "RATCHET_RECURRING_STARTUP_GRACE_SECONDS",
          60L,
          0L);
  static final RatchetConfigKey<Long> RECURRING_CONVERGENCE_WINDOW_SECONDS =
      longKey(
  ...
  static final RatchetConfigKey<Long> RETRY_BUFFER_DRAIN_INTERVAL_MS =
      longKey(
          "ratchet.retry-buffer.drain-interval-ms",
          "RATCHET_RETRY_BUFFER_DRAIN_INTERVAL_MS",
          1000L,
          50L);
  static final RatchetConfigKey<Integer> SOFT_TIMEOUT_PERCENT =
  ...
  static final RatchetConfigKey<String> DLQ_PURGE_CRON =
      stringKey("ratchet.dlq.purge-cron", "RATCHET_DLQ_PURGE_CRON", "0 0 2 * * ?");
  static final RatchetConfigKey<Long> DLQ_PURGE_DAYS =
  ...
  static final RatchetConfigKey<Integer> PRIORITY_BOOST_INTERVAL_MINUTES =
      intKey(
          "ratchet.priority-boost-interval-minutes",
          "RATCHET_PRIORITY_BOOST_INTERVAL_MINUTES",
          15,
          0);
  static final RatchetConfigKey<Boolean> CIRCUIT_BREAKER_ENABLED =
      boolKey("ratchet.circuit-breaker.enabled", "RATCHET_CIRCUIT_BREAKER_ENABLED", true);

  private RatchetConfigKeys() {}

  static RatchetConfigKey<Integer> virtualThreadLimit(String type) {
  ...
  static RatchetConfigKey<Integer> rateLimitPerMinute(String type) {
    String normalized = type.toUpperCase(Locale.ROOT);
    return intKey(
  ...
  static RatchetConfigKey<Float> circuitBreakerFailureRate(String profile, float defaultValue) {
  ...
  static RatchetConfigKey<Integer> circuitBreakerWindowSize(String profile, int defaultValue) {
    String segment = profileSegment(profile);
    return intKey(
  ...
  static RatchetConfigKey<Long> circuitBreakerWaitMs(String profile, long defaultValue) {
    String segment = profileSegment(profile);
    return longKey(
  ...
  static RatchetConfigKey<Integer> circuitBreakerHalfOpenCalls(String profile, int defaultValue) {
    String segment = profileSegment(profile);
    return intKey(
  ...
  static RatchetConfigKey<Integer> circuitBreakerMinimumCalls(String profile, int defaultValue) {
    String segment = profileSegment(profile);
    return intKey(
