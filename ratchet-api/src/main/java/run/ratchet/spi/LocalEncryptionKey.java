 ...
 * instead and pair with an engine that delegates the transform to the service; keeping
 * {@link #material()} off the base type is what lets those adapters drop in without the SPI ever
 * exporting key bytes they cannot produce.
 *
 ...
 * {@link KeyProvider} owns the key's lifecycle and may legitimately cache and reuse a single
 * {@code SecretKey} instance across many operations. An implementation that backs {@link
 * #material()} with a per-call clone, or that intends a key to be zeroized after use, must manage
 * that itself — the framework treats the returned material as borrowed and read-only.
 *
