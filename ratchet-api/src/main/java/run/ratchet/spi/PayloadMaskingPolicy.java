...
/**
 * SPI that decides which payload fields are sensitive when Ratchet builds a {@code JobDetail} for a
 * read API. The policy covers detail parameters, trace context, and a structured result when
 * payload masking is enabled. It is not a general logging filter: applications must still avoid
 * logging raw payloads. The durable store payload and anything a worker needs to execute are never
 * altered.
 *
...
public interface PayloadMaskingPolicy {
   ...
   * @return {@code true} if the field's value should be masked in job-detail read output, {@code
   *     false} otherwise
   */
